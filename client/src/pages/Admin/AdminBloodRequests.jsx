...
import AdminBloodRequestRow from './AdminBloodRequestRow';
const AdminBloodRequests = ({ loadCount }) => {
      ...
      if (result.status === 200) {
        ...
        let mappedData = result.data.map((x) => {
            return {
                ...
                units: x.units,
                message: x.message,
                status: x.received ? "Received" : "Pending"
            }
        })
        ...
        <table className="min-w-full border-collapse md:table block">
          ...
          <tbody className="md:table-row-group block">
            {requests.map((request, index) => (
              <AdminBloodRequestRow key={index} request={request} refreshRequests={getRequests} loadCount={loadCount} />
            ))}
          </tbody>
        </table>
      </div>
